...
#ifdef _CLOSE_AT
    {Cassete::Close_at, { "Закрыли кассету", 130}},
#endif
#ifdef _ERROR_AT
...
#endif
#ifdef _ERROR_AT
                || pnm->iSubItem == Cassete::Error_at
#endif

#ifdef _CLOSE_AT
                || pnm->iSubItem == Cassete::Close_at
#endif
#ifdef _DELETE_AT
...
#endif
#ifdef _CLOSE_AT
                else if(plvdi->item.iSubItem == Cassete::Close_at)         lstrcpy(plvdi->item.pszText, GetDataTimeStr(p.Close_at).c_str());
#endif
#ifdef _DELETE_AT
...
//LRESULT RighDoubletClickCassette(LPARAM lParam)
//{
//    LPNMITEMACTIVATE pnm = (LPNMITEMACTIVATE)lParam;
...
bool UpdateCassette(std::string ss, std::string& vv, int id)
{
    ...
    if(vv.length())
    {
        ssd << "UPDATE cassette SET " << MapCollCassette[id] << " = '" << vv << "' WHERE id = " << CassetteID->Id;
    }
    else
    {
        ssd << "UPDATE cassette SET " << MapCollCassette[id] << " = DEFAULT WHERE id = " << CassetteID->Id;
    }
